...
* Complexity
** Chained operations can build into complex logic
** Hard to test/debug
** So, pull out into separate lambda functions
...
-----
* The type of `s` is inferred (not declared explicitly)
* The type of `lengthOf`
** is a `Function` accepting a single `String` parameter and returning an `Integer`
*** a reference to something that does computation

[source, java]
-----
lengthOf.apply("foo") // returns  3
-----
...
* Express the idea that an API may not return a result
...
=== Using `Optional` values
* `opt.map` - for `Stream` processing without breaking up the chain to check for missing values
